package com.noteaiapp.keyboardai.customnote;
...
public class GeminiNoteActivity extends AppCompatActivity {
    ...
    ImageButton clearDrawingButton,linkCreationButton,pdfUploadButton;
...
// Also add click listener for quick toggle
    ...
    private void openCamera() {
        Intent i = new Intent(GeminiNoteActivity.this, CameraActivity.class);
    ...
    public void registerListeners(){
        ...
        voiceicon.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (ContextCompat.checkSelfPermission(GeminiNoteActivity.this, android.Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {

                    // --- Case 1: Permission is denied, but not permanently. ---
                    if (ActivityCompat.shouldShowRequestPermissionRationale(GeminiNoteActivity.this, android.Manifest.permission.RECORD_AUDIO)) {
                        // Show an explanation to the user via a dialog.
                        new AlertDialog.Builder(GeminiNoteActivity.this)
                                ...
                                .setPositiveButton(R.string.grant_text, new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ...
                                        ActivityCompat.requestPermissions(GeminiNoteActivity.this, new String[]{android.Manifest.permission.RECORD_AUDIO}, PERMISSION_REQUEST_CODE);
                                    }
                                })
                    ...
                    MediaPlayer mp = MediaPlayer.create(GeminiNoteActivity.this, R.raw.googleassistant);
                    if (mp != null) {
                        mp.start();
    ...
    private void drawOnDrawingView() {
        ...
        if (drawable instanceof BitmapDrawable) {
            ...
            if (bitmap != null) {
                Log.d("com.noteaiapp.keyboardai", "Bitmap retrieved from imagesketch. Preparing to draw.");
                ...
                drawingView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                    ...
                    public void onGlobalLayout() {
                        ...
                        if (drawingView.getWidth() > 0 && drawingView.getHeight() > 0) {
                            Log.d("com.noteaiapp.keyboardai", "DrawingView is ready. Setting drawing data.");
                ...
                Log.w("com.noteaiapp.keyboardai", "Could not get Bitmap from imagesketch's drawable.");
            }
        } else {
            Log.w("com.noteaiapp.keyboardai", "imagesketch does not contain a valid BitmapDrawable. Cannot edit.");
        }
    }
            ...
            try {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        // 3. Inform the user
                        Toast.makeText(GeminiNoteActivity.this, "Note translated to " + targetLanguage, Toast.LENGTH_SHORT).show();
                    });
                    ...
                    runOnUiThread(() -> {
                        if (correctionProgressBar != null) correctionProgressBar.setVisibility(View.GONE);
                        Toast.makeText(GeminiNoteActivity.this, "Error: Translation failed.", Toast.LENGTH_SHORT).show();
                    });
                }
            } catch (Exception e) {
                ...
                runOnUiThread(() -> {
                    if (correctionProgressBar != null) correctionProgressBar.setVisibility(View.GONE);
                    Toast.makeText(GeminiNoteActivity.this, "An error occurred.", Toast.LENGTH_SHORT).show();
                });
            }
        });
    }
            ...
            try {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        Toast.makeText(GeminiNoteActivity.this, "Summary appended!", Toast.LENGTH_SHORT).show();
                    });
                    ...
                    runOnUiThread(() -> {
                        correctionProgressBar.setVisibility(View.GONE);
                        Toast.makeText(GeminiNoteActivity.this, "Error: Could not get summary.", Toast.LENGTH_SHORT).show();
                    });
                }
            } catch (Exception e) {
            ...
            try {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        isNoteModified = true; // Mark the note as modified
                        correctionProgressBar.setVisibility(View.GONE);
                        Toast.makeText(GeminiNoteActivity.this, "Spellings corrected!", Toast.LENGTH_SHORT).show();
                    });
                    ...
                    runOnUiThread(() -> {
                        correctionProgressBar.setVisibility(View.GONE);
                        Toast.makeText(GeminiNoteActivity.this, "Error: Could not correct text.", Toast.LENGTH_SHORT).show();
                    });
                }
            } catch (Exception e) {
...
// ADD this new, combined method to Notepad.java
    ...
    public void init(){
        setContentView(R.layout.gemininote_layout);
